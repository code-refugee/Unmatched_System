...
public class CoreAcctRecordConverterImpl implements CoreAcctRecordConverter {
    ...
    public String encode(User user, List<Step> steps) {
        ...
        //username
        XMLMessageNodeInfo nodeInfo02 = messageOperationOld.getXmlNodeInfoById("02");
        //password
        XMLMessageNodeInfo nodeInfo03 = messageOperationOld.getXmlNodeInfoById("03");
        ...
        for (int i = 0; i < steps.size(); i++) {
            ...
            //浅拷贝
            BeanUtils.copyProperties(temp1, messageOperationOld.getXmlNodeInfoById("05"));
            if(i!=steps.size()-1){
                //stepName
                XMLMessageNodeInfo nodeInfo06= messageOperationOld.getXmlNodeInfoById("06");
                //stepIndex
                XMLMessageNodeInfo nodeInfo07= messageOperationOld.getXmlNodeInfoById("07");
                if (nodeInfo06!=null)
                    nodeInfo06.setValue(steps.get(i).getStepName());
                if(nodeInfo07!=null)
                    nodeInfo07.setValue(steps.get(i).getStepIndex()+"");
                //深拷贝
                temp2= messageOperationOld.copy(messageOperationOld.getXmlNodeInfoById("05"));
                tempList.add(temp2);
            }else {
                //stepName
                XMLMessageNodeInfo nodeInfo06= messageOperationOld.getXmlNodeInfoById("06");
